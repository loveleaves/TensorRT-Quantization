...
bool __check_cuda_runtime(cudaError_t code, const char *op, const char *file, int64_t line)
{
    if (code != cudaSuccess)
    {
        const char *err_name = cudaGetErrorName(code);
        const char *err_message = cudaGetErrorString(code);
        printf("runtime error %s:%ld  %s failed. \n  code = %s, message = %s\n", file, line, op, err_name, err_message);
        return false;
    }
    return true;
}
...
unordered_map<string, int64_t> load_vocab(const string &file, bool add_lower_case = true)
{
    ...
    for (int64_t i = 0; i < lines.size(); ++i)
    {
        ...
        if (add_lower_case)
        {
            ...
            {
                for (int64_t j = 0; j < token.size(); ++j)
                {
...
int64_t find_token(const string &token, const unordered_map<string, int64_t> &vocab)
{
    ...
    if (iter == vocab.end())
    {
        if (!token.empty() && token[0] != '[' && token.back() != ']')
        {
            ...
            for (int64_t j = 0; j < new_token.size(); ++j)
            {
...
tuple<vector<string>, vector<tuple<int64_t, int64_t>>> split_words(const string &text)
{
    ...
    int64_t offset = 0;
    int64_t length = text.size();
    unsigned char lab_char[] = {
        // 11111110  11111000  11110000  11100000  11000000  01111111
        0xFE, 0xF8, 0xF0, 0xE0, 0xC0, 0x80};

    int64_t char_size_table[] = {
        6, 5, 4, 3, 2, 0};

    vector<tuple<int64_t, int64_t>> offset_map;
    vector<string> tokens;
    string ascii;
    int64_t state = 0; // 0 none,  1 wait ascii
    int64_t ascii_start = 0;
    while (offset < length)
    {
...
/* 把字符串拆分为词组，汉字单个为一组 */
tuple<vector<string>, vector<tuple<int64_t, int64_t>>> tokenize(const string &text, const unordered_map<string, int64_t> &vocab, int64_t max_length, bool case_to_lower = false)
{

    vector<tuple<int64_t, int64_t>> offset_map;
    vector<tuple<int64_t, int64_t>> offset_newmap;
    vector<string> tokens;
    vector<string> output;
    auto UNK = "[UNK]";
    tie(tokens, offset_map) = split_words(text);

    for (int64_t itoken = 0; itoken < tokens.size(); ++itoken)
    {
        ...
        vector<tuple<int64_t, int64_t>> sub_offsetmap;
        while (start < chars.size())
        {
            int64_t end = chars.size();
            string cur_substr;
            while (start < end)
            {
                ...
                if (case_to_lower)
                {
                    for (int64_t k = 0; k < substr.size(); ++k)
                    {
                        auto &c = substr[k];
                        if (c >= 'A' && c <= 'Z')
                            c = c - 'A' + 'a';
...
vector<int64_t> tokens_to_ids(const vector<string> &tokens, const unordered_map<string, int64_t> &vocab)
{
    vector<int64_t> output(tokens.size());
    for (int64_t i = 0; i < tokens.size(); ++i)
        output[i] = find_token(tokens[i], vocab);
    return output;
}

tuple<vector<int64_t>, vector<int64_t>, vector<tuple<int64_t, int64_t>>, int64_t> align_and_pad(
    const vector<string> &tokens, vector<tuple<int64_t, int64_t>> &offset_map, int64_t pad_size,
    const unordered_map<string, int64_t> &vocab)
{
    auto CLS = find_token("[CLS]", vocab);
    auto SEP = find_token("[SEP]", vocab);
    vector<int64_t> output = tokens_to_ids(tokens, vocab);
    vector<int64_t> mask(pad_size, 1);
...
tuple<vector<int64_t>, vector<int64_t>, vector<tuple<int64_t, int64_t>>, int64_t> make_text_data(const string &text, const unordered_map<string, int64_t> &vocab, int64_t max_length)
{
...
bool build_model()
{
    ...
    if (!parser->parseFromFile("../workspace/ner.onnx", 1))
    {
...
void inference()
{
    ...
    int64_t input_seqlength = 512;
    int64_t input_numel = input_batch * input_seqlength;
    auto vocab = load_vocab("../model/vocab.txt");
    int64_t *input_ids_device = nullptr;
    int64_t *input_mask_device = nullptr;
    checkRuntime(cudaMalloc(&input_ids_device, input_numel * sizeof(int64_t)));
    checkRuntime(cudaMalloc(&input_mask_device, input_numel * sizeof(int64_t)));

    ///////////////////////////////////////////////////
    // letter box
    const char *input = "My name is Clara and I live in Berkeley, California.";
    vector<int64_t> input_ids, attention_mask;
    ...
    ///////////////////////////////////////////////////
    checkRuntime(cudaMemcpyAsync(input_ids_device, input_ids.data(), input_numel * sizeof(int64_t), cudaMemcpyHostToDevice, stream));
    checkRuntime(cudaMemcpyAsync(input_mask_device, attention_mask.data(), input_numel * sizeof(int64_t), cudaMemcpyHostToDevice, stream));

    // 3x3输入，对应3x3输出
    const char *input1_tensor_name = engine->getIOTensorName(0);
    const char *input2_tensor_name = engine->getIOTensorName(1);
    const char *output_tensor_name = engine->getIOTensorName(2);
    auto output_dims = execution_context->getTensorShape(output_tensor_name);
    int64_t output_seqlen = input_seqlength;
    int64_t num_classes = output_dims.d[2];
    int64_t output_numel = input_batch * output_seqlen * num_classes;
    ...
    // 明确当前推理时，使用的数据输入大小
    auto input_ids_dims = execution_context->getTensorShape(input1_tensor_name);
    input_ids_dims.d[0] = input_batch;
    input_ids_dims.d[1] = input_seqlength;
    execution_context->setInputShape(input1_tensor_name, input_ids_dims);

    auto input_mask_dims = execution_context->getTensorShape(input2_tensor_name);
    input_mask_dims.d[0] = input_batch;
    input_mask_dims.d[1] = input_seqlength;
    execution_context->setInputShape(input2_tensor_name, input_mask_dims);

    execution_context->setTensorAddress(input1_tensor_name, input_ids_device);
    execution_context->setTensorAddress(input2_tensor_name, input_mask_device);
    execution_context->setTensorAddress(output_tensor_name, output_data_device);
    ...
    vector<tuple<string, int64_t, int64_t, int64_t>> output_human;
    vector<int64_t> output_labels(sentence_length);
    int64_t state = 0;
    int64_t cur_start = 0;
    int64_t cur_label = 0;
    for (int64_t i = 0; i < sentence_length; ++i)
    {
        float *pword = output_data_host + i * num_classes;
        int64_t label = std::max_element(pword, pword + num_classes) - pword;
        float confidence = pword[label];
        output_labels[i] = label;

        int64_t start, end;
        tie(start, end) = offset_map[i];
        // printf("label = %ld, name = %s, text = ", label, label_name[label]);
        // if(end - start > 0){
    ...
    if (state == 1)
    {
        int64_t start = get<0>(offset_map[cur_start]);
        int64_t end = get<1>(offset_map[sentence_length - 1]);
        output_human.emplace_back(string(input, start, end - start), start, end, cur_label);
        state = 0;
    }
