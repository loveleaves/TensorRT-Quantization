...
bool build_model()
{
    ...
    if (!parser->parseFromFile("../workspace/alpha-pose-136.onnx", 1))
    {
...
void get_preprocess_transform(const cv::Size &image_size, const cv::Rect &box, const cv::Size &net_size, float i2d[6], float d2i[6])
{
    ...
    i2d[3] = 0;
    i2d[4] = scale;
    i2d[5] = -(box_.y - box_.height * 1 * rate + pad_height * 0.5) * scale + net_size.height * 0.5 + scale * 0.5 - 0.5;
...
void inference()
{
    ...
    ///////////////////////////////////////////////////
    // image to float
    auto image = cv::imread("../workspace/gril.jpg");
    ...
    auto output_dims = engine->getTensorShape(output_tensor_name);
