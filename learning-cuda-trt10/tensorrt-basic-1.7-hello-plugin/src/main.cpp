...
// onnx解析器的头文件
#include <onnx-tensorrt-release-10.8-GA/NvOnnxParser.h>
...
bool build_model()
{
    ...
    if (!parser->parseFromFile("../workspace/demo.onnx", 1))
    {
...
void inference()
{
    ...
    nvinfer1::Dims4 input_dims(ib, 1, ih, iw);
    execution_context->setInputShape("image", input_dims);

    // 明确当前推理时，使用的数据输入大小
    execution_context->setTensorAddress("image", input_data_device);
    execution_context->setTensorAddress("output", output_data_device);
