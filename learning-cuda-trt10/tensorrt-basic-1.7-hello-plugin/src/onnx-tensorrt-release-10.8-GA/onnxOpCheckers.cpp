...
#define DECLARE_OP_CHECKER(op)                                                                                         \
    void check##op(ImporterContext* ctx, ::onnx::NodeProto const& node, std::vector<Status>& errors,         \
...
void argMinMaxCheckHelper(
    ImporterContext* ctx, const ::onnx::NodeProto& node, std::vector<Status>& errors, size_t const nodeIndex)
{
    ...
    ImporterContext* ctx, ::onnx::NodeProto const& node, std::vector<Status>& errors, size_t const nodeIndex)
{
    ...
    if (attrs.count("dtype"))
    {
        ...
        if (dtype != ::onnx::TensorProto::FLOAT && dtype != ::onnx::TensorProto::FLOAT16)
        {
            ADD_STATIC_ERROR(
...
void emptyOutputChecker(ImporterContext* ctx, ::onnx::NodeProto const& node, std::vector<Status>& errors,
...
// define empty checker
DEFINE_OP_EMPTY_CHECKER(MYSELU)
