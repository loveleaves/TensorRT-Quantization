...
public:
    ...
    bool isFp32() const
    {
        return is_tensor() ? tensor().getType() == nvinfer1::DataType::kFLOAT
                           : weights().type == ::onnx::TensorProto_DataType_FLOAT;
    }
    bool isFp16() const
    {
        return is_tensor() ? tensor().getType() == nvinfer1::DataType::kHALF
                    : weights().type == ::onnx::TensorProto_DataType_FLOAT16;
    }
    bool isBFp16() const
    {
        return is_tensor() ? tensor().getType() == nvinfer1::DataType::kBF16
                    : weights().type == ::onnx::TensorProto_DataType_BFLOAT16;
    }
    bool isInt32() const
    {
        return is_tensor() ? tensor().getType() == nvinfer1::DataType::kINT32
                           : weights().type == ::onnx::TensorProto_DataType_INT32;
    }
    bool isInt64() const
    {
        return is_tensor() ? tensor().getType() == nvinfer1::DataType::kINT64
                           : weights().type == ::onnx::TensorProto_DataType_INT64;
    }
    bool isInt8() const
    {
        return is_tensor() ? tensor().getType() == nvinfer1::DataType::kINT8
                           : weights().type == ::onnx::TensorProto_DataType_INT8;
    }
    bool isBool() const
    {
        return is_tensor() ? tensor().getType() == nvinfer1::DataType::kBOOL : weights().type == ::onnx::TensorProto_DataType_BOOL;
    }
    bool isFp8() const
    {
        return is_tensor() ? tensor().getType() == nvinfer1::DataType::kFP8 : weights().type == ::onnx::TensorProto_DataType_FLOAT8E4M3FN;
    }
