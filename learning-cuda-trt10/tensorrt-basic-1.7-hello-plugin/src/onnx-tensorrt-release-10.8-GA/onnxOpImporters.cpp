...
#define DECLARE_BUILTIN_OP_IMPORTER(op)                                                                                \
    NodeOutputs import##op(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t const nodeIdx,        \
...
NodeOutputs batchnormFallback(
    ImporterContext* ctx, ::onnx::NodeProto const& node, size_t nodeIdx, std::vector<TensorOrWeights>& inputs)
{
    ...
    if (varType == DataType::kHALF)
    {
        epsLayer = addConstantScalar(
            ctx, static_cast<half_float::half>(eps), ::onnx::TensorProto::FLOAT16, scalarShape);
    }
    else if (varType == DataType::kBF16)
    {
        epsLayer
            = addConstantScalar(ctx, static_cast<BFloat16>(eps), ::onnx::TensorProto::BFLOAT16, scalarShape);
    }
    else
    {
        epsLayer = addConstantScalar(ctx, eps, ::onnx::TensorProto::FLOAT, scalarShape);
    }
...
DEFINE_BUILTIN_OP_IMPORTER(BatchNormalization)
{
    ...
    int32_t const nbChannels = scale.shape.d[0];
    auto combinedScale
        = ctx->createNamedTempWeights(::onnx::TensorProto::FLOAT, scale.shape, /*batchNormNode=*/true);
    auto combinedBias
        = ctx->createNamedTempWeights(::onnx::TensorProto::FLOAT, bias.shape, /*batchNormNode=*/true);
...
DEFINE_BUILTIN_OP_IMPORTER(BlackmanWindow)
{
    ...
    ***/
    ...
    auto betaTensor = N_CHECK(addConstantScalar(ctx, beta, ::onnx::TensorProto_DataType_FLOAT,
    ...
    auto gammaTensor = N_CHECK(addConstantScalar(ctx, gamma, ::onnx::TensorProto_DataType_FLOAT,
        nvinfer1::Dims{1, {1}})->getOutput(0));
    auto gammaLayer
    ...
    auto alphaTensor = N_CHECK(addConstantScalar(ctx, alpha, ::onnx::TensorProto_DataType_FLOAT,
        nvinfer1::Dims{1, {1}})->getOutput(0));
    auto alphaMinusBeta
...
DEFINE_BUILTIN_OP_IMPORTER(Celu)
{
    ...
    TensorOrWeights weightsOfZero = ctx->createNamedTempWeights(::onnx::TensorProto::FLOAT, {0, {}});
    ShapedWeights weightsOfOnes = ctx->createNamedTempWeights(::onnx::TensorProto::FLOAT, {0, {}});
    std::vector<float> ones{1};
    std::memcpy(weightsOfOnes.values, ones.data(), weightsOfOnes.count() * sizeof(float));
    ShapedWeights weightsOfAlpha = ctx->createNamedTempWeights(::onnx::TensorProto::FLOAT, {0, {}});
...
NodeOutputs elementwiseClipHelper(ImporterContext* ctx, ::onnx::NodeProto const& node,
    ...
    if (elementwiseClip)
    {
        ...
        if (type == DataType::kHALF)
        {
            return elementwiseClipHelper<half_float::half>(
                ctx, node, inputs, numInputs, ::onnx::TensorProto::FLOAT16);
        }
        if (type == DataType::kBF16)
        {
            return elementwiseClipHelper<BFloat16>(
                ctx, node, inputs, numInputs, ::onnx::TensorProto::BFLOAT16);
        }
        ...
        {
            return elementwiseClipHelper<int64_t>(ctx, node, inputs, numInputs, ::onnx::TensorProto::INT64);
        }
...
DEFINE_BUILTIN_OP_IMPORTER(Constant)
{
    ...
    if (ctx->getOpsetVersion() >= 12)
    {
        ...
        {
            ShapedWeights convertedWeights = ctx->createNamedTempWeights(::onnx::TensorProto::FLOAT, {0, {}});
        ...
        if (attrs.count("value_floats"))
        {
            ...
            ShapedWeights convertedWeights
                = ctx->createNamedTempWeights(::onnx::TensorProto::FLOAT, {1, {valueSize}});
            std::memcpy(convertedWeights.values, values.data(), convertedWeights.count() * sizeof(float));
            return {{convertedWeights}};
        }
        if (attrs.count("value_int"))
        {
            ShapedWeights convertedWeights = ctx->createNamedTempWeights(::onnx::TensorProto::INT64, {0, {}});
        ...
        if (attrs.count("value_ints"))
        {
            ...
            int32_t valueSize = values.size();
            ShapedWeights convertedWeights
                = ctx->createNamedTempWeights(::onnx::TensorProto::INT64, {1, {valueSize}});
...
DEFINE_BUILTIN_OP_IMPORTER(ConstantOfShape)
{
    ...
    ShapedWeights zeroWeights
        = ctx->createNamedTempWeights(::onnx::TensorProto_DataType_FLOAT, nvinfer1::Dims{1, {1}});
...
NodeOutputs QuantDequantLinearHelper(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t nodeIdx,
    std::vector<TensorOrWeights>& inputs, bool isDQ, bool isCustomOp, DataType customOpType = DataType::kFP8)
{
    ...
    auto const outputDTypeOnnx = attrs.get<int32_t>("output_dtype", ::onnx::TensorProto::UNDEFINED);
    bool isOutputDtypeSet = (outputDTypeOnnx != ::onnx::TensorProto::UNDEFINED);
    if (isOutputDtypeSet)
    {
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_FP4DynamicQuantize)
{
    ...
    auto const blockSizeOnnx = attrs.get<int32_t>("block_size", ::onnx::TensorProto::UNDEFINED);
    auto const scaleTypeOnnx = attrs.get<int32_t>("scale_type", ::onnx::TensorProto::UNDEFINED);

    DataType scaleType;
    bool isScaleTypeSet = (scaleTypeOnnx != ::onnx::TensorProto::UNDEFINED);
    if (isScaleTypeSet)
    {
    ...
    int32_t blockSize = -1;
    bool isBlockSizeSet = (blockSizeOnnx != ::onnx::TensorProto::UNDEFINED);
    if (isBlockSizeSet)
    {
...
DEFINE_BUILTIN_OP_IMPORTER(EyeLike)
{
    ...
    ShapedWeights tempWeights = ctx->createNamedTempWeights(::onnx::TensorProto::INT32, dims);
...
DEFINE_BUILTIN_OP_IMPORTER(GlobalLpPool)
{
    ...
    if (inputType == DataType::kHALF)
    {
        pLayer = addConstantScalar(
            ctx, static_cast<half_float::half>(p), ::onnx::TensorProto::FLOAT16, scalarDims);
        pInvLayer = addConstantScalar(
            ctx, static_cast<half_float::half>(1.F / p), ::onnx::TensorProto::FLOAT16, scalarDims);
    }
    else
    {
        pLayer = addConstantScalar(ctx, p, ::onnx::TensorProto::FLOAT, scalarDims);
        pInvLayer = addConstantScalar(ctx, 1.F / p, ::onnx::TensorProto::FLOAT, scalarDims);
    }
...
nvinfer1::ITensor* concatenateRNNOutputs(ImporterContext* ctx, ::onnx::NodeProto const& node,
...
DEFINE_BUILTIN_OP_IMPORTER(GRU)
{
    ...
    auto* constOne = zt->getType() == nvinfer1::DataType::kHALF
        ? N_CHECK(addConstantScalar(
              ctx, static_cast<half_float::half>(1), ::onnx::TensorProto::FLOAT16, Dims3{1, 1, 1})
                      ->getOutput(0))
        : N_CHECK(addConstantScalar(ctx, 1.f, ::onnx::TensorProto::FLOAT, Dims3{1, 1, 1})->getOutput(0));
    ...
    nvinfer1::ITensor* singlePassShape = getElementWiseResult(ctx, *gateOutputShape,
        *addConstant(ctx, std::vector<int32_t>{numDirections, 1, 1}, ::onnx::TensorProto_DataType_INT32,
            nvinfer1::Dims{1, {3}})
             ->getOutput(0),
...
DEFINE_BUILTIN_OP_IMPORTER(ImageScaler)
{
    ...
    ShapedWeights shiftWeights = ctx->createNamedTempWeights(::onnx::TensorProto_DataType_FLOAT, dims);
    ...
    ShapedWeights scaleWeights = ctx->createNamedTempWeights(::onnx::TensorProto_DataType_FLOAT, dims);
...
DEFINE_BUILTIN_OP_IMPORTER(LayerNormalization)
{
    ...
    {
        scaleLayer = addConstantScalar(ctx, static_cast<half_float::half>(1), ::onnx::TensorProto::FLOAT16);
        biasLayer = addConstantScalar(ctx, static_cast<half_float::half>(0), ::onnx::TensorProto::FLOAT16);
    }
    else if (dt == DataType::kBF16)
    {
        scaleLayer = addConstantScalar(ctx, static_cast<BFloat16>(0), ::onnx::TensorProto::BFLOAT16);
        biasLayer = addConstantScalar(ctx, static_cast<BFloat16>(0), ::onnx::TensorProto::BFLOAT16);
    }
    else
    {
        scaleLayer = addConstantScalar(ctx, static_cast<float>(0), ::onnx::TensorProto::FLOAT);
        biasLayer = addConstantScalar(ctx, static_cast<float>(0), ::onnx::TensorProto::FLOAT);
    }
    ...
    for (int32_t i = nbStateVars + NB_DISCARDED_OUTPUTS; i < nbOutputs; ++i)
    {
        ...
        else
        {
            trtScanOut->setInput(1,
                *N_CHECK(addConstantScalar(ctx, DUMMY_SCAN_OUTPUT_LENGTH, ::onnx::TensorProto_DataType_INT32)
                             ->getOutput(0)));
        }
...
DEFINE_BUILTIN_OP_IMPORTER(LSTM)
{
    ...
    auto const initialStateShape = [&ctx, &numDirections, &hiddenSize, &input]() -> nvinfer1::ITensor* {
        ...
        nvinfer1::ITensor* numDirectionsTensor = addConstantScalar(
            ctx, numDirections, ::onnx::TensorProto_DataType_INT32, nvinfer1::Dims{1, {1}})
                                                     ->getOutput(0);
...
DEFINE_BUILTIN_OP_IMPORTER(LpPool)
{
    ...
    if (dt == DataType::kHALF)
    {
        kernelSzLayer = addConstantScalar(
            ctx, static_cast<half_float::half>(kernelSz), ::onnx::TensorProto::FLOAT16, scalarDims);
    }
    else
    {
        kernelSzLayer = addConstantScalar(ctx, kernelSz, ::onnx::TensorProto::FLOAT, scalarDims);
    }
...
DEFINE_BUILTIN_OP_IMPORTER(Mean)
{
    ...
    auto scale_dtype = sum_input.isBFp16()
        ? ::onnx::TensorProto::BFLOAT16
        : (sum_input.isFp16() ? ::onnx::TensorProto::FLOAT16 : ::onnx::TensorProto::FLOAT);
...
DEFINE_BUILTIN_OP_IMPORTER(MeanVarianceNormalization)
{
    ...
    std::fill(scalarShape.d, scalarShape.d + scalarShape.nbDims, 1);
    auto* epsilonTensor
        = addConstantScalar(ctx, 1e-9f, ::onnx::TensorProto_DataType_FLOAT, scalarShape)->getOutput(0);
...
DEFINE_BUILTIN_OP_IMPORTER(NonMaxSuppression)
{
    ...
    if (inputs.size() >= 3 && !inputs.at(2).isNullTensor())
    {
        ...
        nvinfer1::ITensor* int32Max = addConstantScalar(
            ctx, static_cast<int64_t>(std::numeric_limits<int32_t>::max()), ::onnx::TensorProto::INT64)
                                          ->getOutput(0);
        ...
        else if (value != 0.F)
        {
            ...
            case DataType::kHALF:
                fillValue = addConstantScalar(
                    ctx, static_cast<half_float::half>(value), ::onnx::TensorProto::FLOAT16)
                                ->getOutput(0);
                break;
            case DataType::kBF16:
                fillValue
                    = addConstantScalar(ctx, static_cast<BFloat16>(value), ::onnx::TensorProto::BFLOAT16)
                          ->getOutput(0);
                break;
            case DataType::kFLOAT:
            case DataType::kINT8:
                fillValue = addConstantScalar(ctx, value, ::onnx::TensorProto::FLOAT)->getOutput(0);
                break;
            default:
                fillValue = addConstantScalar(ctx, static_cast<int32_t>(value), ::onnx::TensorProto::INT32)
                                ->getOutput(0);
                break;
            }
            ONNXTRT_CHECK_NODE(
...
NodeOutputs randomHelper(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t nodeIdx,
    ShapeTensor const& inputShape, OnnxAttrs const& attrs, DataType const& inputDType, nvinfer1::FillOperation op)
{
    ...
    if (attrs.count("dtype"))
    {
        ...
        switch (dtype)
        {
        case ::onnx::TensorProto::FLOAT: fillLayer->setToType(DataType::kFLOAT); break;
        case ::onnx::TensorProto::FLOAT16: fillLayer->setToType(DataType::kHALF); break;
...
DEFINE_BUILTIN_OP_IMPORTER(RNN)
{
    ...
    auto const getInitialInputValue
        ...
        return constantOfShape(ctx,
            N_CHECK(addConstantScalar(ctx, 0.f, ::onnx::TensorProto_DataType_FLOAT, nvinfer1::Dims{1, {1}})
                        ->getOutput(0)),
            initialStateShape());
    };
    ...
    nvinfer1::ITensor* singlePassShape = getElementWiseResult(ctx, *initialStateShape(),
        *N_CHECK(addConstant(ctx, std::vector<int>{numDirections, 1, 1}, ::onnx::TensorProto_DataType_INT32,
            nvinfer1::Dims{1,
                {3}})->getOutput(0)),
        nvinfer1::ElementWiseOperation::kDIV);
...
NodeOutputs scatterPluginHelper(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t nodeIdx,
...
DEFINE_BUILTIN_OP_IMPORTER(Squeeze)
{
    ...
    else
    {
        ...
        if (attrs.count("axes"))
        {
            ...
            axesTensor = N_CHECK(
                addConstant(ctx, axes, ::onnx::TensorProto::INT64, {1, {static_cast<int64_t>(axes.size())}})
                    ->getOutput(0));
        }
    }
...
DEFINE_BUILTIN_OP_IMPORTER(STFT)
{
    ...
    */
    ...
    int64_t frameStep{0};
    ShapedWeights windowWeights = ShapedWeights::empty(::onnx::TensorProto_DataType_FLOAT);
    ...
    if (frameLength != 0 && windowWeights.values == nullptr)
    {
        windowWeights = ctx->createNamedTempWeights(::onnx::TensorProto_DataType_FLOAT, {1, {frameLength}});
    ...
    */
    ...
    auto realWeights = ctx->createNamedTempWeights(
        ::onnx::TensorProto_DataType_FLOAT, {4, {dftUniqueBins, 1, 1, frameLength}});
    ...
    if (ctx->getOpsetVersion() >= 9)
    {
        ...
        if (scales_input.is_weights())
        {
            ...
            ONNXTRT_CHECK_NODE((scales_weights.count() == static_cast<size_t>(nbDims)),
                ...
                node, nodeIdx, ErrorCode::kINVALID_NODE);
            ONNXTRT_CHECK_NODE((scales_weights.type == ::onnx::TensorProto::FLOAT),
    ...
    nvinfer1::PluginFieldCollection const* fieldNames, ImporterContext* ctx)
{
    ...
    for (int32_t i = 0; i < fieldNames->nbFields; ++i)
    {
        ...
        case ::onnx::AttributeProto::TENSOR:
        {
            ...
            case ::onnx::TensorProto::BFLOAT16: type = nvinfer1::PluginFieldType::kBF16; break;
            ...
            case ::onnx::TensorProto::COMPLEX128:
                // ::onnx::TensorProto::DataType_Name function not available in protobuf-lite.
                std::stringstream ss{};
                ss << "Tensor type: ";
#if USE_LITE_PROTOBUF
                ss << static_cast<::onnx::TensorProto::DataType>(tensor.type);
#else
                ss << ::onnx::TensorProto::DataType_Name(
        ...
        case ::onnx::AttributeProto::TYPE_PROTOS:
            // ::onnx::AttributeProto::AttributeType_Name function not available in protobuf-lite.
            std::stringstream ss{};
            ss << "Attributes of type: ";
#if USE_LITE_PROTOBUF
            ss << attrs.type(fieldName);
#else
            ss << ::onnx::AttributeProto::AttributeType_Name(attrs.type(fieldName));
#endif // USE_LITE_PROTOBUF
...
NodeOutputs addPluginWithCreator(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t const& nodeIdx,
    ...
    nvinfer1::IPluginCreatorInterface* creator)
{
    ...
    if (attrs.count("tensorrt_plugin_shape_input_indices"))
    {
        ...
        {
            ONNXTRT_CHECK_NODE(
                attrs.type("tensorrt_plugin_shape_input_indices") == ::onnx::AttributeProto::INTS,
...
DEFINE_BUILTIN_OP_IMPORTER(LocalFunctionImporter)
{
    ...
    StringMap<::onnx::AttributeProto const*> attrMap;
...
DEFINE_BUILTIN_OP_IMPORTER(MYSELU)
{
    ...
    nvinfer1::IPluginCreatorInterface *creator = importPluginCreator(ctx, pluginName, pluginVersion, pluginNamespace);
    ONNXTRT_CHECK_NODE(creator, "Plugin not found, are the plugin name, version, and namespace correct?", node, nodeIdx, ErrorCode::kUNSUPPORTED_NODE);
    ...
    default:
        ONNXTRT_CHECK(false && "Unsupported plugin creator version.", ErrorCode::kUNSUPPORTED_NODE);
    }
}
