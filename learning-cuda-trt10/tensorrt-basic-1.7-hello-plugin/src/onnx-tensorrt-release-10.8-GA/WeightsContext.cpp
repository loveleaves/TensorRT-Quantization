...
int32_t* WeightsContext::convertUINT8(uint8_t const* weightValues, nvinfer1::Dims const& shape)
{
    ...
    int32_t* int32Weights{
        static_cast<int32_t*>(createTempWeights(::onnx::TensorProto::INT32, shape).values)};
...
float* WeightsContext::convertDouble(double const* weightValues, nvinfer1::Dims const& shape)
{
    ...
    int64_t const nbWeights = volume(shape);
    float* floatWeights{
        static_cast<float*>(createTempWeights(::onnx::TensorProto::FLOAT, shape).values)};
...
bool validateOnnxInitializer(::onnx::TensorProto const& onnxTensor)
{
    // Validate type.
...
bool WeightsContext::convertOnnxWeights(
    ::onnx::TensorProto const& onnxTensor, ShapedWeights* weights, bool ownAllWeights)
{
    auto* ctx = this; // For logging macros.
    ...
    if (dataLocation == 1)
    {
        ...
        if (onnxDtype == ::onnx::TensorProto::UINT8)
        {
    ...
    else if (onnxDtype == ::onnx::TensorProto::INT32 || onnxDtype == ::onnx::TensorProto::INT64
        || onnxDtype == ::onnx::TensorProto::FLOAT16 || onnxDtype == ::onnx::TensorProto::BFLOAT16
    ...
    else if (onnxDtype == ::onnx::TensorProto::FLOAT8E4M3FN)
    {
        if (onnxTensor.raw_data().size() > 0)
        {
            dataPtr = (void*) (onnxTensor.raw_data().data());
...
{
    int64_t const nbWeights = volume(shape);
    float* newWeights{static_cast<float*>(createTempWeights(::onnx::TensorProto::FLOAT, shape).values)};
...
{
    assert((w.type == ::onnx::TensorProto::FLOAT || w.type == ::onnx::TensorProto::FLOAT16)
