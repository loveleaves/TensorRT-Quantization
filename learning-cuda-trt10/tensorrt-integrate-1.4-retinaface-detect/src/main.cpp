...
bool build_model()
{
    ...
    if (!parser->parseFromFile("../workspace/mb_retinaface.onnx", 1))
    {
...
void inference()
{
    ...
    // 3x3输入，对应3x3输出
    const char *input_tensor_name = engine->getIOTensorName(0);   // 获取输入张量名称
    const char *output_tensor_name = engine->getIOTensorName(1);   // 获取输出张量名称
    auto output_dims = engine->getTensorShape(output_tensor_name); // 获取张量维度
    ...
    // 明确当前推理时，使用的数据输入大小
    auto input_dims = execution_context->getTensorShape(input_tensor_name);
    input_dims.d[0] = input_batch;
    execution_context->setInputShape(input_tensor_name, input_dims);

    execution_context->setTensorAddress(input_tensor_name, input_data_device);
    ...
    // nms
    std::sort(bboxes.begin(), bboxes.end(), [](Face &a, Face &b)
