...
onnx2trt::ShapedWeights OnnxAttrs::get<onnx2trt::ShapedWeights>(std::string const& key) const
{
    ...
    ::onnx::TensorProto const& onnxTensor = isExtAttr ? mCtx->localFunctionStack().back().attrs.at(extName)->t() : this->at(key)->t();
...
template <>
const ::onnx::GraphProto& OnnxAttrs::get<const ::onnx::GraphProto&>(std::string const& key) const
{
