...
nvinfer1::ITensor* addLoopCounter(ImporterContext* ctx, nvinfer1::ILoop* loop, int64_t initial)
{
    nvinfer1::ITensor* initialTensor
        = addConstantScalar(ctx, initial, ::onnx::TensorProto::INT64, nvinfer1::Dims{1, {1}})->getOutput(0);
    nvinfer1::ITensor* one = addConstantScalar(ctx, static_cast<int64_t>(1), ::onnx::TensorProto::INT64,
