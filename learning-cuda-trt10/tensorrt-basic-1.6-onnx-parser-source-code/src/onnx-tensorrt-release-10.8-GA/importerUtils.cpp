...
Status notInvalidType(TensorOrWeights const& input, std::vector<std::string> const& invalidTypes,
    ::onnx::NodeProto const& node, size_t const nodeIdx)
{
...
NodeOutputs activationHelper(ImporterContext* ctx, const ::onnx::NodeProto& node, size_t const nodeIdx,
...
NodeOutputs argMinMaxHelper(ImporterContext* ctx, const ::onnx::NodeProto& node, size_t const nodeIdx,
...
void convertAxis(int32_t& axis, int32_t const nbDims, ::onnx::NodeProto const& node, size_t const nodeIdx)
{
...
bool convertDtype(int32_t onnx_dtype, nvinfer1::DataType* trt_dtype)
{
    ...
    case ::onnx::TensorProto::BFLOAT16: *trt_dtype = nvinfer1::DataType::kBF16; break;
    ...
    case ::onnx::TensorProto::INT64: *trt_dtype = nvinfer1::DataType::kINT64; break;
    case ::onnx::TensorProto::FLOAT8E4M3FN: *trt_dtype = nvinfer1::DataType::kFP8; break;
    case ::onnx::TensorProto::INT4: *trt_dtype = nvinfer1::DataType::kINT4; break;
    case ::onnx::TensorProto::FLOAT4E2M1: *trt_dtype = nvinfer1::DataType::kFP4; break;
    default:
    ...
    nvinfer1::ITensor*& startTensor, nvinfer1::ITensor*& totalPaddingTensor)
{
    ...
    auto* startLayer
        = N_CHECK(addConstant(ctx, start, ::onnx::TensorProto::INT64, nvinfer1::Dims{1, {nbInputDims}}));
    startTensor = N_CHECK(startLayer->getOutput(0));

    auto* totalPaddingLayer = N_CHECK(
        addConstant(ctx, totalPadding, ::onnx::TensorProto::INT64, nvinfer1::Dims{1, {nbInputDims}}));
...
nvinfer1::ITensor* createZeroTensor(ImporterContext* ctx, nvinfer1::ITensor* data)
{
    ...
    auto* zeros = N_CHECK(addConstantScalar(ctx, 0.0F, ::onnx::TensorProto::FLOAT)->getOutput(0));
...
NodeOutputs elementwiseHelper(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t const nodeIdx,
...
nvinfer1::ITensor* flattenTensor(
    ImporterContext* ctx, ::onnx::NodeProto const& node, nvinfer1::ITensor& tensor, int axis, bool regLayer)
{
    auto const dims = shapeOf(tensor);
...
void getKernelParams(ImporterContext* ctx, ::onnx::NodeProto const& node, nvinfer1::Dims* kernelSize,
...
float getSingleValueAsFloat(ShapedWeights const& weights)
{
    ...
    switch (weights.type)
    {
    case ::onnx::TensorProto::FLOAT: return static_cast<float const*>(weights.values)[0];
...
nvinfer1::ITensor* globalPoolingHelper(ImporterContext* ctx, ::onnx::NodeProto const& node,
...
NodeOutputs greaterLessOrEqual(ImporterContext* ctx, const ::onnx::NodeProto& node, size_t const nodeIdx,
...
std::unique_ptr<nvinfer1::IPluginV2, PluginDeleter> createPlugin(ImporterContext* ctx,
    ::onnx::NodeProto const& node, std::string const& name, std::string const& /* pluginNamespace */,
...
std::unique_ptr<nvinfer1::IPluginV3> createPlugin(ImporterContext* ctx, ::onnx::NodeProto const& node,
...
NodeOutputs staticSliceImporter(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t const nodeIdx,
...
NodeOutputs modulatedDeformableConvPluginHelper(ImporterContext* ctx, ::onnx::NodeProto const& node,
...
NodeOutputs instanceNormPluginHelper(ImporterContext* ctx, ::onnx::NodeProto const& node,
    size_t const nodeIdx, std::vector<TensorOrWeights>& inputs)
{
...
nvinfer1::ITensor* iota(ImporterContext* ctx, ShapeTensor iotaDims, int32_t axis)
{
    ...
    auto* alphaLayer = N_CHECK(addConstantScalar(ctx, static_cast<int32_t>(0), ::onnx::TensorProto::INT32));
    auto* alpha = N_CHECK(alphaLayer->getOutput(0));
    auto* deltaLayer
        = N_CHECK(addConstant(ctx, deltaVals, ::onnx::TensorProto::INT32, {1, {iotaDims.size()}}));
...
NodeOutputs normalizationHelper(ImporterContext* ctx, const ::onnx::NodeProto& node, size_t const nodeIdx,
...
NodeOutputs poolingHelper(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t const nodeIdx,
...
bool IsReduceNoOp(
    ImporterContext* ctx, ::onnx::NodeProto const& node, std::vector<TensorOrWeights> const& inputs)
{
    OnnxAttrs attrs(node, ctx);
    return (attrs.get("noop_with_empty_axes", 0) == 1) && (!attrs.count("axes")) && (inputs.size() == 1);
}

NodeOutputs reduceTensor(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t const nodeIdx,
...
NodeOutputs scaleHelper(ImporterContext* ctx, const ::onnx::NodeProto& node, size_t const nodeIdx,
...
void setAttr(nvinfer1::Dims* trtAttr, ::onnx::AttributeProto const* onnxAttr, int32_t nbSpatialDims,
    int32_t defaultVal)
{
...
nvinfer1::ITensor* sliceAcrossAxis(
    ImporterContext* ctx, const ::onnx::NodeProto& node, nvinfer1::ITensor* data, int32_t const axis)
{
...
{
    auto* axesTensor
        = N_CHECK(addConstant(ctx, axes, ::onnx::TensorProto::INT32, {1, {static_cast<int64_t>(axes.size())}})
                      ->getOutput(0));
...
nvinfer1::ITensor* transposeTensor(ImporterContext* ctx, const ::onnx::NodeProto& node,
...
::onnx::TensorProto_DataType trtDataTypeToONNX(nvinfer1::DataType dt)
{
    switch (dt)
    {
...
NodeOutputs unaryHelper(ImporterContext* ctx, const ::onnx::NodeProto& node, size_t const nodeIdx,
    TensorOrWeights& input, nvinfer1::UnaryOperation op)
{
    ...
    case nvinfer1::UnaryOperation::kNEG:
    {
        ...
        if (inputType == nvinfer1::DataType::kINT32)
        {
            ...
            auto* zeroLayer
                = N_CHECK(addConstant(ctx, std::vector<int32_t>{0}, ::onnx::TensorProto::INT32, {0, {1}}));
...
NodeOutputs convMultiInput(ImporterContext* ctx, const ::onnx::NodeProto& node, size_t const nodeIdx,
    std::vector<TensorOrWeights>& inputs)
{
    ...
    auto kernelWeights = ShapedWeights::empty(::onnx::TensorProto::FLOAT);
...
std::string const getNodeName(::onnx::NodeProto const& node)
{
...
nvinfer1::ITensor* addSoftmax(
    ImporterContext* ctx, const ::onnx::NodeProto& node, size_t const nodeIdx, nvinfer1::ITensor& input)
{
    OnnxAttrs attrs(node, ctx);
...
NodeOutputs addScatterLayer(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t const nodeIdx,
...
void processMetadata(ImporterContext* ctx, ::onnx::NodeProto const& node, nvinfer1::ILayer* layer)
{
...
nvinfer1::IEinsumLayer* parseGraphWithMoreInputs(ImporterContext* ctx, ::onnx::NodeProto const& node,
...
{
    auto* numeratorTensor = N_CHECK(addConstantScalar(ctx, numerator, ::onnx::TensorProto_DataType_FLOAT,
    ...
    if (periodic == 0)
    {
        auto* one = N_CHECK(addConstantScalar(ctx, 1, ::onnx::TensorProto_DataType_INT32)->getOutput(0));
