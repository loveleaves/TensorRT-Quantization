...
nvinfer1::ITensor* flattenTensor(ImporterContext* ctx, ::onnx::NodeProto const& node,
...
NodeOutputs normalizationHelper(ImporterContext* ctx, ::onnx::NodeProto const& node, size_t const nodeIdx,
    std::vector<TensorOrWeights>& inputs);
...
void weightsToVector(TensorOrWeights weights, std::vector<WeightType>* weightVector)
{
    ...
    ONNXTRT_CHECK((weights.weights().type == ::onnx::TensorProto::INT32)
            || (weights.weights().type == ::onnx::TensorProto::INT64)
            || (weights.weights().type == ::onnx::TensorProto::BOOL)
            || (weights.weights().type == ::onnx::TensorProto::FLOAT),
    ...
    else if (weights.weights().type == ::onnx::TensorProto::FLOAT)
    {
...
NodeOutputs addScatterLayer(ImporterContext* ctx, const ::onnx::NodeProto& node, size_t const nodeIdx,
